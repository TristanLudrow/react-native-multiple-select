...
export default class MultiSelect extends Component<_t_props, _t_state> {
  ...
  _getInputTextColor = () => {
    ...
    if (invalid && !selector) inputColor = errorColor;

    return inputColor;
  };

  _getRow = (item) => {
    const { selectedItemTextColor, displayKey, dropdownTextStyle, dropdownItemStyle } = this.props;
  ...
  _renderDropdown = () => {
    ...
    let component = null;
    let itemList;
    const renderItems = searchTerm && !disableNativeFilter ? this._filterItems(searchTerm) : items;
    ...
    return (
      <View>
        <TouchableOpacity
          ...
          activeOpacity={1}
          hitSlop={inputContainerHitSlop ? inputContainerHitSlop : null}
        >
          <TextInput
